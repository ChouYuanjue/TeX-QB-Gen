...
利用 OpenRouter 提供的多模态/文本模型与本地 OCR 能力，从图片、网页或 PDF 中抽取数学题目与解答，并生成统一格式的 TeX 题库。每道题会生成独立的 TeX 文件，并由 `master.tex` 汇总方便批量编译。
...
- [ ] 生成正确且美观的`master.tex`文件，包含目录、章节甚至参考文献等等。
- [ ] 处理明显的错误返回结果。比如一次返回多题结果、未正确返回json、LLM生成的答案实际为Markdown语法等等。
- [ ] 对扫描版 PDF 进行测试。
- [ ] 对答案出现在书后的情况进行测试。
